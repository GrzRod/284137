...
void drukuj_obj(ISerialize* obj)
{
...
class Student : public Osoba, public ISerialize
{
private:
    int nr_indeksu;
public:
...
class Pracownik : public Osoba, public ISerialize
{
private:
    int id_pracownika;
public:
...
std::string Pracownik::serialize()
{
...
std::string ListaObecnosci::serialize()
{
    std::string out;
    for (int i = 0; i < liczba; i++)
    {
        if (tabOsob[i] != nullptr)
        {
            ISerialize* s = dynamic_cast<ISerialize*>(tabOsob[i]);
            std::string person = s ? s->serialize() : "Osoba bez serializacji";
            out += person + " | Obecnosc: " + (tabObecnosci[i] ? "Tak" : "Nie") + "\n";
        }
    }
    return out;
}
...
void InterfejsUzytkownika::zapis()
{
    std::ofstream ofs("zapis.txt");
    if (!ofs)
    {
        std::cout << "Nie mozna utworzyc pliku do zapisu.\n";
        return;
    }
    ...
    for (int i = 0; i < maxOsob; i++)
    {
        if (bazyOsob[i] != nullptr)
        {
            ISerialize* s = dynamic_cast<ISerialize*>(bazyOsob[i]);
            if (s)
                ofs << s->serialize() << "\n";
            else
                ofs << "Osoba bez serializacji: ID=" << bazyOsob[i]->getId() << "\n";
        }
    }

    ofs << "\nListyObecnosci (aktywnych: " << aktywneListy << "):\n";
    for (int i = 0; i < aktywneListy; i++)
    {
        ofs << "Lista " << (i + 1) << ":\n";
        ofs << tabList[i].serialize() << "\n";
    }

    ofs.close();
    std::cout << "Zapisano do pliku 'zapis.txt'.\n";
}
